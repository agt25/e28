...
class FavoriteRequest extends FormRequest
{
    ...
    public function authorize()
    {
        ...
        # If this favorite is not user restricted, then everyone is authorized
        if (!$favorite->userRestricted) {
            return true;
        }
        ...
        # If the request includes an id, we need to find that favorite by id
        # and check that its user_id matches the id of the requesting user
        if (!is_null($this->route('id'))) {
            return $this->user()->id == $favorite->find($this->route('id'))->user_id;
        }
